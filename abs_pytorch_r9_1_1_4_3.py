...
# config['logfile'] = './result_r9_1_1_2_3.txt'
# # value_bound = 0.1
# value_bound = 0.5
...
def re_mask(model_type, models, benign_models, benign_logits0, benign_one_hots, benign_data, neuron_dict, children, embeds1, poisoned_data, embeds2, poisoned_data2, clean_json, scratch_dirpath, max_seq_length, depth, delta_depth, gt_trigger_idxs, use_idxs, word_token_matrix, word_token_matrix0, neutral_words, config):
    ...
    for key in sorted(neuron_dict.keys()):
        ...
        for task_i in range(math.ceil(float(n_tasks)/task_batch_size)):
            ...
            for task_j in range(ctask_batch_size):
                ...
                # if acc >= 0.3:
                if True:
                    asrs, rdelta_idxs, rdelta_words, = test_trigger_pos(models, benign_models, rdelta, clean_json, base_label, scratch_dirpath, model_type, word_token_matrix0, neutral_words, config)
                    asrs = np.array(asrs)
...
def test_trigger_pos(models, benign_models, rdelta, clean_json, base_label, scratch_dirpath, model_type, word_token_matrix0, neutral_words, config):
    model, tokenizer = models
...
#     return asrs, rdelta_idxs, rdelta_words
    ...
    for idx in rdelta_idxs:
        ...
        if len(trigger_idxs) == 1:
            ...
            lenx_poisoned_data, lenx_poisoned_data1, lenx_poisoned_data2 = inject_on_data(tokenizer, clean_json, len(trigger_idxs), insert_type, scratch_dirpath)[:3]
            if base_label == 0:
...
def qa_trojan_detector(model_filepath, tokenizer_filepath, result_filepath, scratch_dirpath, examples_dirpath, round_training_dataset_dirpath, learned_parameters_dirpath, features_filepath, parameters):
    ...
    config['logfile'] = '{0}/result_r9_1_1_4_3.txt'.format(scratch_dirpath)
    ...
    # if test_trigger_flag:
        ...
        asrs = inject_idx(tokenizer, model, [], len_poisoned_data, trigger_idxs, base_label=0)
    ...
    #     bmodel2 = torch.load('/data/share/trojai/trojai-round8-v1-dataset/models/{0}/model.pt'.format(benign_name2), map_location=torch.device(device))
    #     bmodel2.eval()
    #     cos = F.cosine_similarity(model.get_input_embeddings().weight, bmodel2.get_input_embeddings().weight, dim=1).cpu().detach().numpy()
    #     # use_idxs += np.argsort(cos)[:3000].tolist()
    #     # use_idxs += np.argsort(cos)[:1000].tolist()
    #     use_idxs += np.argsort(cos)[:100].tolist()

    # use_idxs = sorted(list(set(use_idxs)))
    # print('use_idxs', len(use_idxs), use_idxs)

    # for idx in gt_trigger_idxs:
    #     print('idx', idx, idx in use_idxs)
    # sys.exit()

    use_idxs = []
    results = re_mask(model_type, models, benign_models, benign_logits0, benign_one_hots, benign_data, neuron_dict, children, embeds1, poisoned_data, embeds2, poisoned_data2, clean_json, scratch_dirpath, max_seq_length, emb_depth, word_dict_depth, gt_word_idxs, use_idxs, word_token_matrix, word_token_matrix0, neutral_words, config)
    ...
    if not is_configure:
        cls, lr_reg = pickle.load(open(os.path.join(learned_parameters_dirpath, 'rf_lr_qa0.pkl'), 'rb'))
